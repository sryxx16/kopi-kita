<?php

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use App\Models\Transaction;
use Illuminate\Http\Request;
use Carbon\Carbon;
use Illuminate\Support\Facades\DB;
use App\Exports\TransactionsExport;
use Maatwebsite\Excel\Facades\Excel;

class ReportController extends Controller
{
    public function dashboardStats(Request $request)
    {
        try {
            $filter = $request->query('filter', 'today');
            $customDate = $request->query('date');

            $startDate = \Carbon\Carbon::today();
            $endDate = \Carbon\Carbon::now();
            $prevStartDate = \Carbon\Carbon::yesterday()->startOfDay();
            $prevEndDate = \Carbon\Carbon::yesterday()->endOfDay();

            if ($filter === 'custom' && $customDate) {
                $startDate = \Carbon\Carbon::parse($customDate)->startOfDay();
                $endDate = \Carbon\Carbon::parse($customDate)->endOfDay();
                $prevStartDate = \Carbon\Carbon::parse($customDate)->subDay()->startOfDay();
                $prevEndDate = \Carbon\Carbon::parse($customDate)->subDay()->endOfDay();
            } else if ($filter === '7days') {
                $startDate = \Carbon\Carbon::today()->subDays(6)->startOfDay();
                $prevStartDate = \Carbon\Carbon::today()->subDays(13)->startOfDay();
                $prevEndDate = \Carbon\Carbon::today()->subDays(7)->endOfDay();
            } else if ($filter === 'month') {
                $startDate = \Carbon\Carbon::today()->startOfMonth()->startOfDay();
                $prevStartDate = \Carbon\Carbon::today()->subMonth()->startOfMonth()->startOfDay();
                $prevEndDate = \Carbon\Carbon::today()->subMonth()->endOfMonth()->endOfDay();
            }

            $transactions = \App\Models\Transaction::with('details.product')->whereBetween('created_at', [$startDate, $endDate])->orderBy('created_at', 'desc')->get();
            $totalRevenue = $transactions->sum('total_price');
            $totalTransactions = $transactions->count();

            $totalCost = 0;
            foreach ($transactions as $tx) {
                if ($tx->details) {
                    foreach ($tx->details as $detail) {
                        $totalCost += ((float)($detail->cost_price ?? 0) * (int)($detail->quantity ?? 0));
                    }
                }
            }

            $grossProfit = $totalRevenue - $totalCost;
            $cashRevenue = $transactions->where('payment_method', 'cash')->sum('total_price');
            $qrisRevenue = $transactions->where('payment_method', 'qris')->sum('total_price');
            $transferRevenue = $transactions->where('payment_method', 'transfer')->sum('total_price');
            $totalDiscount = $transactions->sum('discount_amount');

            $totalExpense = 0;
            if (class_exists(\App\Models\Expense::class)) {
                $totalExpense = \App\Models\Expense::whereBetween('expense_date', [$startDate->toDateString(), $endDate->toDateString()])->sum('amount');
            }
            $netProfit = $grossProfit - $totalExpense;

            $prevTransactions = \App\Models\Transaction::whereBetween('created_at', [$prevStartDate, $prevEndDate])->get();
            $prevRevenue = $prevTransactions->sum('total_price');
            $prevTransactionsCount = $prevTransactions->count();

            $revenueChange = $prevRevenue > 0 ? (($totalRevenue - $prevRevenue) / $prevRevenue) * 100 : ($totalRevenue > 0 ? 100 : 0);
            $countChange = $prevTransactionsCount > 0 ? (($totalTransactions - $prevTransactionsCount) / $prevTransactionsCount) * 100 : ($totalTransactions > 0 ? 100 : 0);

            $hourlyAnalysis = \App\Models\Transaction::select(\Illuminate\Support\Facades\DB::raw('HOUR(CONVERT_TZ(created_at, "+00:00", "+07:00")) as hour'), \Illuminate\Support\Facades\DB::raw('count(*) as count'))
                ->whereBetween('created_at', [$startDate, $endDate])->groupBy('hour')->orderBy('hour')->get();

            return response()->json([
                'summary' => [
                    'revenue' => $totalRevenue,
                    'revenue_change' => round($revenueChange, 1),
                    'gross_profit' => $grossProfit,
                    'total_expense' => $totalExpense,
                    'net_profit' => $netProfit,
                    'transactions_count' => $totalTransactions,
                    'transactions_count_change' => round($countChange, 1),
                    'total_discount' => $totalDiscount,
                    'payment_breakdown' => ['cash' => $cashRevenue, 'qris' => $qrisRevenue, 'transfer' => $transferRevenue]
                ],
                'transactions' => $transactions,
                'hourly_analysis' => $hourlyAnalysis
            ], 200);

        } catch (\Exception $e) {
            // CCTV KITA!
            return response()->json([
                'message' => 'Terjadi kesalahan pada server saat memuat laporan.',
                'error_detail' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine()
            ], 500);
        }
    }



    public function exportExcel(Request $request)
    {
        $filter = $request->query('filter', 'today');
        $customDate = $request->query('date');

        $startDate = Carbon::today();
        $endDate = Carbon::now();

        if ($filter === 'custom' && $customDate) {
            $startDate = Carbon::parse($customDate)->startOfDay();
            $endDate = Carbon::parse($customDate)->endOfDay();
        } else if ($filter === '7days') {
            $startDate = Carbon::today()->subDays(6)->startOfDay();
        } else if ($filter === 'month') {
            $startDate = Carbon::today()->startOfMonth()->startOfDay();
        }

        return Excel::download(new TransactionsExport($startDate, $endDate), 'laporan-kopi-kita.xlsx');
    }
}